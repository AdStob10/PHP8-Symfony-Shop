 ...
 */
final class Version20210522142356 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__order AS SELECT id, status, status_date, created_date, client_name, client_address FROM "order"');
        $this->addSql('DROP TABLE "order"');
        $this->addSql('CREATE TABLE "order" (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER DEFAULT NULL, status INTEGER NOT NULL, status_date DATETIME DEFAULT NULL, created_date DATETIME DEFAULT NULL, client_name VARCHAR(150) DEFAULT NULL COLLATE BINARY, client_address VARCHAR(200) DEFAULT NULL COLLATE BINARY, CONSTRAINT FK_F5299398A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO "order" (id, status, status_date, created_date, client_name, client_address) SELECT id, status, status_date, created_date, client_name, client_address FROM __temp__order');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__order AS SELECT id, status, created_date, status_date, client_name, client_address FROM "order"');
        $this->addSql('DROP TABLE "order"');
        $this->addSql('CREATE TABLE "order" (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, status INTEGER NOT NULL, created_date DATETIME DEFAULT NULL, status_date DATETIME DEFAULT NULL, client_name VARCHAR(150) DEFAULT NULL, client_address VARCHAR(200) DEFAULT NULL)');
        $this->addSql('INSERT INTO "order" (id, status, created_date, status_date, client_name, client_address) SELECT id, status, created_date, status_date, client_name, client_address FROM __temp__order');
        $this->addSql('DROP TABLE __temp__order');
        $this->addSql('DROP INDEX IDX_2530ADE64584665A');
        $this->addSql('DROP INDEX IDX_2530ADE6A5862391');
        $this->addSql('CREATE TEMPORARY TABLE __temp__order_product AS SELECT id, product_id, order_obj_id, quantity FROM order_product');
