 ...
 */
final class Version20210527170441 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__order AS SELECT id, user_id, status, status_date, created_date, client_name, client_city, client_street, client_post_code FROM "order"');
        $this->addSql('DROP TABLE "order"');
        $this->addSql('CREATE TABLE "order" (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER DEFAULT NULL, status INTEGER NOT NULL, status_date DATETIME DEFAULT NULL, created_date DATETIME DEFAULT NULL, client_name VARCHAR(150) DEFAULT NULL COLLATE BINARY, client_city VARCHAR(200) DEFAULT NULL COLLATE BINARY, client_street VARCHAR(150) DEFAULT NULL COLLATE BINARY, client_post_code VARCHAR(9) DEFAULT NULL COLLATE BINARY, client_country VARCHAR(2) DEFAULT NULL, CONSTRAINT FK_F5299398A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO "order" (id, user_id, status, status_date, created_date, client_name, client_city, client_street, client_post_code) SELECT id, user_id, status, status_date, created_date, client_name, client_city, client_street, client_post_code FROM __temp__order');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('DROP INDEX IDX_F5299398A76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__order AS SELECT id, user_id, status, created_date, status_date, client_name, client_city, client_street, client_post_code FROM "order"');
        $this->addSql('DROP TABLE "order"');
        ...
        $this->addSql('INSERT INTO "order" (id, user_id, status, created_date, status_date, client_name, client_city, client_street, client_post_code) SELECT id, user_id, status, created_date, status_date, client_name, client_city, client_street, client_post_code FROM __temp__order');
        $this->addSql('DROP TABLE __temp__order');
        $this->addSql('CREATE INDEX IDX_F5299398A76ED395 ON "order" (user_id)');
        $this->addSql('DROP INDEX IDX_2530ADE64584665A');
        $this->addSql('DROP INDEX IDX_2530ADE6A5862391');
